...
class HotelHelper
{
    ...
    public function insertDefaultHotelEntries()
    {
        ...
        Configuration::updateValue('WK_HEADER_MEDIA_TYPE', HotelHeaderMedia::MEDIA_TYPE_IMAGE);
        Configuration::updateValue('WK_HEADER_SLIDER_NAV_TYPE', HotelHeaderMedia::NAV_TYPE_DOTS);
        Configuration::updateValue('WK_HEADER_SLIDER_AUTO_PLAY', 1);
        Configuration::updateValue('WK_HEADER_SLIDER_INTERVAL', 5000);
        Configuration::updateValue('WK_HEADER_SLIDER_ANIM_TYPE', HotelHeaderMedia::ANIM_TYPE_SLIDE);
        ...
        if (file_exists($mediaDir.$defaultImgDest)) {
            Db::getInstance()->execute(
                'INSERT INTO `'._DB_PREFIX_.'htl_header_media`
                (`media_type`, `name`, `source_type`, `position`, `active`, `date_add`, `date_upd`)
                VALUES (\'image\', \''.pSQL($defaultImgDest).'\', \'upload\', 0, 1, NOW(), NOW())'
            );
            ...
            if ($newId) {
                ...
                foreach (Language::getLanguages(false) as $lang) {
                    ...
                    Db::getInstance()->execute(
                        'INSERT INTO `'._DB_PREFIX_.'htl_header_media_lang`
                        (`id_header_media`, `id_lang`, `tag_line`)
